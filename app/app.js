...
const Q_ALL = [
  ...
  {
    /* Canada only. NOAA's US normals carry no sunshine element, so every one of
       the 4,197 US places returned "not recorded here" while the tile sat there
       looking pickable. A tile that can never answer is worse than a missing
       one: it spends a pick and hands back a winner chosen on nothing. */
    id: 'sun', cc: ['CA'], g: 'Climate', label: 'Sun', col: 'Sun', hint: 'Hours of it a year.',
...
/* A phone can hand a file to the system share sheet; a desktop cannot, and gets
   the clipboard instead. Decided once, and the button says which, because
   "Save the picture" promised a file and delivered a Save As dialog. */
const CAN_SHARE_FILES = !!(navigator.canShare && navigator.canShare(
  { files: [new File([new Uint8Array(1)], 'p.png', { type: 'image/png' })] }));
const SHARE_LABEL = CAN_SHARE_FILES ? 'Send the picture' : 'Copy the picture';

/* The picture IS the product - the job is getting it into a chat. A download
   interrupts that with a filename and a folder picker and then leaves the thing
   in Downloads to go and find. Share sheet on a phone, clipboard on a desktop,
   and if both are refused, the card on screen to long-press. No save dialog on
   any path. */
function shareCard() {
  const btn = $('#sharecard');
  const say = (t) => { if (!btn) return; btn.textContent = t;
    setTimeout(() => { btn.textContent = SHARE_LABEL; }, 2600); };

  // The blob is requested but NOT awaited before asking for the clipboard:
  // Safari only honours a write requested inside the click that caused it, and
  // awaiting the canvas first spends the gesture. ClipboardItem takes the promise.
  const blobP = drawCard().then((c) => c && new Promise((res) => c.toBlob(res, 'image/png')));

  if (!CAN_SHARE_FILES && window.ClipboardItem && navigator.clipboard && navigator.clipboard.write) {
    navigator.clipboard.write([new ClipboardItem({ 'image/png': blobP })])
      .then(() => say('Copied. Paste it anywhere.'))
      .catch(() => blobP.then(showCardSheet));
    return;
  }
  blobP.then(async (blob) => {
    if (!blob) return;
...
// Last resort, and still not a dialog: the card on screen, theirs to take.
function showCardSheet(blob) {
  if (!blob) return;
  const url = URL.createObjectURL(blob);
  $('#sheetBody').innerHTML =
    `<img class="card-img" src="${url}" alt="Your result as a picture" width="1080" height="1350">
     <p class="card-hint">Press and hold, or right-click, to save it.</p>`;
...
function actionbar() {
  ...
  if (screen === 'pick') {
    ...
    bar.innerHTML = `<button class="ab-back" id="abExplore">See all ${DATA.length}</button>
      <button class="ab-main" id="sharecard">${SHARE_LABEL}</button>`;
  } else {
